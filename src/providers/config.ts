    ...
    switch (config.type) {
        ...
        case 'openai':
            ...
            if (typeof config.apiKey !== 'string' || config.apiKey.trim() === '') {
                throw new ProviderConfigError('OpenAI API key must be a non-empty string');
            }
            if (config.temperature !== undefined && (typeof config.temperature !== 'number' || config.temperature < 0 || config.temperature > 2)) {
                throw new ProviderConfigError('OpenAI temperature must be a number between 0 and 2');
            }
            break;
        ...
        case 'anthropic':
            ...
            if (typeof config.apiKey !== 'string' || config.apiKey.trim() === '') {
                throw new ProviderConfigError('Anthropic API key must be a non-empty string');
            }
            if (config.temperature !== undefined && (typeof config.temperature !== 'number' || config.temperature < 0 || config.temperature > 1)) {
                throw new ProviderConfigError('Anthropic temperature must be a number between 0 and 1');
            }
            break;
