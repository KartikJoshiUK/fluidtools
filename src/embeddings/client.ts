...
export class EmbeddingClient {
    ...
    constructor(modalUrl: string, debug: boolean = false) {
        ...
        logger(this.debug, "🔧 [EmbeddingClient] Initialized with URL:", this.modalUrl);
    }
    ...
    async indexTools(sessionId: string, tools: Tool[]): Promise<void> {
        logger(this.debug, `📊 [EmbeddingClient.indexTools] Indexing ${tools.length} tools for session ${sessionId}`);
